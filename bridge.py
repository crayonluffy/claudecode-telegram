...
BOT_COMMANDS = [
    ...
    {"command": "pick", "description": "Select option N from prompt"},
...
TMUX SESSIONS
  ...
  /attach <name> - Switch to session
  /start [name] [dir] - Create new session
...
QUICK REPLY
  /y /n /ok /retry
  /pick N - Select option from interactive prompt
...
def tmux_restart_claude(session=None, start_dir=None):
    ...
    if not tmux_exists(session):
        return tmux_create(session, start_dir, start_claude=True)

    # Session exists - try to start Claude
    # First send Escape to cancel any current input
    ...
    def handle_message(self, update):
        ...
        if text.startswith("/"):
            ...
            if cmd == "/restart":
                ...
                success, msg = tmux_restart_claude(start_dir)
                if success:
                    self.reply(chat_id, f"✅ Claude Code restarted")
                else:
                    self.reply(chat_id, f"❌ {msg}")
                return
